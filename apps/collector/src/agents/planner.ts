...
export function planEntry(source: SourceConfig, state: SourceState, now: number): PlanEntry {
  ...
  if (state.status === "broken") {
    return { sourceId: source.id, action: "heal", reason: `validator flagged drift: ${state.lastError ?? "unknown"}` };
  }
  if (state.status === "healing") {
    return { sourceId: source.id, action: "run", reason: "post-heal re-run" };
  }
  if (state.status === "unbuilt") {
    return { sourceId: source.id, action: "run", reason: `registered collector ${state.collectorId}, first run` };
  }
  if (!state.lastRunAt || now - state.lastRunAt > staleAt) {
    return { sourceId: source.id, action: "run", reason: `stale (last run ${new Date(state.lastRunAt as number).toISOString()})` };
  }
